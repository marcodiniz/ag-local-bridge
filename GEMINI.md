...
### Streaming Modes (`agLocalBridge.streamingMode`)

Configurable strategy for handling streaming (`stream: true`) requests:

- **`smart` (Default)**: Automatically detects request characteristics:
  - Pure chat streaming (no tools) → uses **Cascade Pseudo-Streaming** (`callSidecarChatStream`) for instant token-by-token SSE delivery and low TTFB.
  - Requests with `tools` (function-calling) or non-streaming → uses **Raw Inference** (`callRawInference`) so client tools execute properly.
- **`raw`**: Strictly forces all requests through `GetModelResponse` (pure LLM completions, full swarm account pooling, full response buffered before emission).
- **`cascade`**: Always routes streaming requests through Cascade trajectory delta polling.
